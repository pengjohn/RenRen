/*!
*@FileName: RenRenForm_BlogGetComments.cpp
...
// 构造函数
TBlogGetCommentsForm::TBlogGetCommentsForm(TApplication* pApp, Boolean bNew):TWindow(pApp)
{
	m_TitleBtnLeft = -1;
	m_TitleBtnRight = -1;
	m_MoreBtn = -1;
...
// 析构函数
TBlogGetCommentsForm::~TBlogGetCommentsForm(void)
{
	RenRenAPICommon_DownloadCancelByHwndId(this->GetWindowHwndId());	

	for(int i=0;i<BLOGGETCOMMENTS_MAXITEM ;i++)
	{
...
// 窗口事件处理
Boolean TBlogGetCommentsForm::EventHandler(TApplication * pApp, EventType * pEvent)
{
	...
	switch (pEvent->eType)
	{
		...
		case MSG_DL_THREAD_NOTIFY:
			{
				...
				switch(notifyData.nAccessType)
				{
					...
					case RR_BlogAddComment:
					{
						pApp->MessageBox(TResource::LoadConstString(APP_RE_ID_STRING_Success), TResource::LoadConstString(APP_RE_ID_STRING_Comment), WMB_OK);
					...
					{
						tResponseBlogGetComments* Response = NULL;
						RenRenAPI_JsonParse(RR_BlogGetCommentsMore, (void **)&Response);
						if(Response != NULL)
						{
		...
		case MSG_POST_THREAD_NOTIFY:
		{
				...
				char* id = Get_Url_Params(RR_BlogGetComments, "id");
				char* uid = Get_Url_Params(RR_BlogGetComments, "uid");
				Set_Url_Params(RR_BlogAddComment, "id", id);
				Set_Url_Params(RR_BlogAddComment, "uid", uid);
				//notifyData.nParam = -1表示发起新评论
				if(notifyData.nParam == -1)
					Set_Url_Params(RR_BlogAddComment, "rid", "");
				else
				{
					char rid[32] = {0};
					sprintf(rid, "%d", gItemData[notifyData.nParam].uid);
					Set_Url_Params(RR_BlogAddComment, "rid", rid);
				}
				if(notifyData.bPrivacy == TRUE)
					Set_Url_Params(RR_BlogAddComment, "type", "1");
				else
					Set_Url_Params(RR_BlogAddComment, "type", "0");
				
				Set_Url_Params(RR_BlogAddComment, "content", (char*)notifyData.pszEditContent);			

				RenRenAPICommon_Download(RR_BlogAddComment, this->GetWindowHwndId(), 0, 0);		
				bHandled = TRUE;
			break;
		}
		default:
...
// 窗口初始化
Boolean TBlogGetCommentsForm::_OnWinInitEvent(TApplication * pApp, EventType * pEvent)
{
	int iRet = eFailed;
	tResponseBlogGetComments* Response = NULL;
	...
	SetAppTilte(this, APP_RE_ID_STRING_BlogComment);

	//从对应Json中获取数据,用于更新UI
	iRet = RenRenAPI_JsonParse(RR_BlogGetComments, (void **)&Response);	
...
// 关闭窗口时，保存设置信息
Boolean TBlogGetCommentsForm::_OnWinClose(TApplication * pApp, EventType * pEvent)
{
	return TRUE;
}

// 控件点击事件处理
Boolean TBlogGetCommentsForm::_OnCtrlSelectEvent(TApplication * pApp, EventType * pEvent)
{
	Boolean bHandled;
	bHandled = FALSE;
	...
	{
		char* szPage = Get_Url_Params(RR_BlogGetCommentsMore, "page");
		char szPageParams[16] = {0};
		int nPage = 0;
		nPage = atoi(szPage);
		nPage ++;
		sprintf(szPageParams, "%d", nPage);
		Set_Url_Params(RR_BlogGetCommentsMore, "page", szPageParams);
		RenRenAPICommon_Download(RR_BlogGetCommentsMore, this->GetWindowHwndId(), 0, 0);
		bHandled = TRUE; 
		return bHandled;			
	}
...
//控件获取焦点
Boolean TBlogGetCommentsForm::_OnCtrlSetFocusEvent(TApplication * pApp, EventType * pEvent)
{
	Boolean bHandled = FALSE;
	Int32 nCtrlID = pEvent->sParam1; 
...
//控件丢失焦点
Boolean TBlogGetCommentsForm::_OnCtrlKillFocusEvent(TApplication * pApp, EventType * pEvent)
{
	Boolean bHandled = FALSE;
...
// 文本框输入处理事件
Boolean TBlogGetCommentsForm::_OnFldChangedEvent(TApplication * pApp, EventType* pEvent)
{
 ...
 * \return 
 */
Boolean TBlogGetCommentsForm::_UpdateList(TApplication * pApp, tResponseBlogGetComments* Response)
{
	if( Response == NULL)
	{
		return FALSE;
	...
	for(int i=0; i<Response->nArraySize; i++)
	{
		...
		TUString::StrUtf8ToStrUnicode(BubbleItemData.pszContent , (const Char *)Response->Array[i].content);
		...
		//悄悄话
		if( strcmp(Response->Array[i].is_whisper, "1") == 0)
		{
	...
	//如果还有,显示更多
	char*  szCount = Get_Url_Params(RR_BlogGetComments, "count");
	int nCount = 0;
	nCount = atoi(szCount);
